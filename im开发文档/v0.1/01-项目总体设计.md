...
- **脚手架状态**：✅ 已完成开发，且已支持 **CookieCutter 模板**
- **脚手架提供**：项目基础结构、配置管理、数据库连接、日志工具、ID 生成器等通用基础设施
- **本项目开发**：业务模型、业务服务、业务 API、AI 服务集成等业务逻辑

**脚手架使用方式**（⭐ 推荐使用 CookieCutter）：
```bash
# 1. 安装 CookieCutter
...
# 3. 按提示输入项目信息（选择 include_celery=y, include_websocket=y）
```

详细说明见：`04-开发计划.md` 中的"脚手架依赖说明"章节。
...
- **Celery 配置**：使用脚手架 Celery 配置（如果包含）
- **WebSocket 管理器**：使用脚手架 WebSocket 管理器（如果包含）

### 10.2 业务模块开发

本项目需要开发的业务模块：

- **数据模型**：Message、AIChatRecord
- **Schema**：MessageCreate、MessageResponse、AIChatRecordResponse
- **Repository**：MessageRepository、AIChatRecordRepository
- **服务层**：MessageService、AIService、WebSocketService
- **API 路由**：消息 API、AI API
- **Celery 任务**：AI 回复任务
- **WebSocket 处理器**：WebSocket 业务逻辑处理
