...
**Python 版本**：**3.11**（固定版本，不允许模糊版本）

**环境要求**：
- Conda 环境：`py311_gb25_im`
- Python 版本：3.11（必须）
- pip 可用
...
- [x] 确认 conda 环境已创建：`conda activate py311_gb25_im`
- [x] 验证 Python 版本：`python --version`（确认为 3.11）
- [x] 验证 pip 可用：`pip list`
...
**任务 1.2：项目初始化（使用 CookieCutter 模板）** ✅ **已完成**

- [x] 安装 CookieCutter：`pip install cookiecutter`
- [x] 使用脚手架模板生成项目（或直接复制脚手架代码）
- [x] 项目目录结构已创建
- [x] 检查项目结构是否完整
...
**任务 1.3：配置扩展（基于脚手架配置）** ✅ **已完成**

**当前状态**：
- ✅ `app/config.py` 已包含AI相关配置（`IMSettings`类）
- ✅ 配置格式已更新为新设计格式（`ai_service_url`, `ai_service_api_key`, `ai_service_timeout`）
- ✅ `.env.example` 已更新，包含火山引擎DeepSeek-R1的配置示例
- ✅ 已创建测试脚本验证配置可以正常加载

**待完成任务**：
- [x] 更新 `app/config.py`，将AI配置更新为新设计格式：
  ```python
  # AI服务配置（AI封装层使用）
  ...
  ```
- [x] 更新 `.env.example`，添加AI服务配置示例（使用火山引擎DeepSeek-R1）：
  ```
  AI_SERVICE_URL=https://ark.cn-beijing.volces.com/api/v3/chat/completions
  AI_SERVICE_API_KEY=your_api_key_here
  AI_SERVICE_TIMEOUT=30
  ```
- [x] 验证配置加载正常：创建测试脚本验证配置可以正常读取（`cursor_test/test_ai_config.py`）

**验收标准**：
- ✅ `app/config.py` 包含新格式的AI服务配置项（`ai_service_url`, `ai_service_api_key`, `ai_service_timeout`）
- ✅ `.env.example` 包含AI服务配置示例（火山引擎DeepSeek-R1）
- ✅ 配置可以正常加载（测试脚本验证通过）

---

**任务 1.4：数据库表创建（基于脚手架数据库模块）** ✅ **已完成**

**当前状态**：
- ✅ `app/models/user.py` 已更新，符合新设计
- ✅ `app/models/message.py` 已更新，符合新设计
- ✅ `app/models/ai_chat_record.py` 已重命名为 `app/models/ai_interaction_record.py` 并更新字段
- ✅ `app/models/group.py` 已创建
- ✅ `app/models/group_member.py` 已创建
- ✅ 数据库迁移脚本已创建

**已完成任务**：
- [x] 更新 `app/models/user.py`，按照新设计实现：
  - 字段：`user_id`, `username`, `user_role`（PATIENT/DOCTOR/AI_ASSISTANT）
  - 索引：`idx_user_id`, `idx_user_role`
  - 方法：`is_patient()`, `is_doctor()`, `is_ai_assistant()`
- [x] 创建 `app/models/group.py`：
  - 字段：`group_id`, `group_name`, `description`, `created_by`
  - 索引：`idx_group_id`, `idx_created_by`
- [x] 创建 `app/models/group_member.py`：
  - 字段：`group_id`, `user_id`, `user_role`, `joined_at`
  - 联合唯一约束：`uk_group_user`
  - 索引：`idx_group_id`, `idx_user_id`, `idx_user_role`
- [x] 更新 `app/models/message.py`，按照新设计实现：
...
- [x] 重命名并更新 `app/models/ai_chat_record.py` 为 `app/models/ai_interaction_record.py`：
  - 字段：`record_id`, `group_id`, `user_message_id`, `ai_message_id`, `ai_service_url`, `request_content`, `response_content`, `status`, `duration_ms`, `error_message`
- [x] 更新 `app/models/__init__.py`，导出所有模型：
  ...
  ```
- [x] 更新 `alembic/env.py`，确保导入所有模型（已更新导入语句）
- [x] 创建数据库迁移脚本：
  ```bash
  alembic revision --autogenerate -m "Create initial tables"
  ```
  - 迁移脚本文件：`alembic/versions/20251210_1554_25c8f48abee5_create_initial_tables.py`
- [x] 检查生成的迁移脚本，确认包含所有表：
  - ✅ `users` - 包含字段：user_id, username, user_role，索引：idx_user_id, idx_user_role
  - ✅ `groups` - 包含字段：group_id, group_name, description, created_by，索引：idx_group_id, idx_created_by
  - ✅ `group_members` - 包含字段：group_id, user_id, user_role, joined_at，联合唯一约束：uk_group_user，索引：idx_group_id, idx_user_id, idx_user_role
  - ✅ `messages` - 包含字段：message_id, group_id, from_user_id, msg_type, msg_content，索引：idx_group_id, idx_from_user_id, idx_created_at, idx_group_created
  - ✅ `ai_interaction_records` - 包含所有必需字段和索引
...
- [x] 执行数据库迁移：
  ...
  ```
  - ✅ 迁移脚本已成功执行
  - ✅ 修复了索引冲突问题（使用条件检查避免重复创建索引）
- [x] 验证数据库表创建成功：
  - ✅ 使用 `scripts/verify_tables.py` 脚本验证
  - ✅ 所有 5 个表都已成功创建：
    - `users` - 6 个字段, 2 个索引
    - `groups` - 7 个字段, 3 个索引
    - `group_members` - 7 个字段, 4 个索引
    - `messages` - 8 个字段, 4 个索引
    - `ai_interaction_records` - 13 个字段, 6 个索引

**验收标准**：
- ✅ 所有模型文件已创建并符合新设计
- ✅ `app/models/__init__.py` 导出所有模型
- ✅ 数据库迁移脚本已创建
- ✅ 数据库表创建成功（5个表都存在，所有字段和索引正确）

---

**任务 1.5：主应用扩展（基于脚手架主应用）** ✅ **已完成**

- [x] 检查 `app/main.py`（脚手架提供的基础版本）
- [x] 验证健康检查接口正常（已实现 `/health` 接口）
- [x] WebSocket路由已注册（`/ws/{user_id}`）
- [x] 预留业务路由注册位置（已在 main.py 中添加注释说明，待后续里程碑实现）

**验收标准**：
- ✅ 项目可以正常启动：`uvicorn app.main:app --reload`
- ✅ 健康检查接口返回正常：`/health` 接口已实现
- ✅ 日志输出正常（使用脚手架日志工具）
- ✅ 业务路由注册位置已预留（main.py 中已添加注释）
...
- [x] 实现 `app/models/user.py`：
  - ✅ 字段：`user_id`, `username`, `user_role`
  - ✅ 索引：`idx_user_id`, `idx_user_role`
  - ✅ 方法：`is_patient()`, `is_doctor()`, `is_ai_assistant()`
- [x] 实现 `app/models/group.py`：
  - ✅ 字段：`group_id`, `group_name`, `description`, `created_by`
  - ✅ 索引：`idx_group_id`, `idx_created_by`
- [x] 实现 `app/models/group_member.py`：
  - ✅ 字段：`group_id`, `user_id`, `user_role`, `joined_at`
  - ✅ 联合唯一约束：`uk_group_user`
  - ✅ 索引：`idx_group_id`, `idx_user_id`, `idx_user_role`
- [x] 实现 `app/models/message.py`：
  - ✅ 字段：`message_id`, `group_id`, `from_user_id`, `msg_type`, `msg_content`
  - ✅ 索引：`idx_group_id`, `idx_from_user_id`, `idx_created_at`, `idx_group_created`
- [x] 验证所有模型继承自 `BaseModel`（脚手架提供）
...
**任务 2.3：Repository层实现** ✅ **已完成**

- [x] 实现 `app/repositories/user_repository.py`：
  - ✅ `create(user_id, username, user_role) -> User`
  - ✅ `get_by_id(user_id) -> Optional[User]`
  - ✅ `update(user) -> User`
  - ✅ 额外方法：`get_by_role(user_role) -> List[User]`
- [x] 实现 `app/repositories/group_repository.py`：
  - ✅ `create(group_id, group_name, description, created_by) -> Group`
  - ✅ `get_by_id(group_id) -> Optional[Group]`
  - ✅ `update(group) -> Group`
  - ✅ 额外方法：`get_by_creator(created_by) -> List[Group]`
- [x] 实现 `app/repositories/group_member_repository.py`：
  ...
  - ✅ 额外方法：`get_groups_by_user(user_id)`, `get_members_by_role(group_id, user_role)`
- [x] 实现 `app/repositories/message_repository.py`：
  ...
  - ✅ 额外方法：`get_by_user(from_user_id)`, `get_recent_messages(group_id, limit)`
- [x] 验证所有Repository使用脚手架提供的ID生成器（已在 `id_generator.py` 中添加便捷函数）
...
- [x] 数据库表创建成功（5个表）✅
- [x] Repository可以正常进行CRUD操作✅
- [x] Schema验证正常✅
- [x] 所有模型、Schema、Repository可以正常导入✅
...
- [x] 实现 `app/services/core/user_service.py`：
...
- [x] 验证用户身份验证逻辑（PATIENT/DOCTOR/AI_ASSISTANT）
- [x] 验证错误处理（用户不存在、无效user_role等）
...
- [x] 实现 `app/services/core/group_service.py`：
...
- [x] 验证创建群组时自动添加创建人为成员（身份为DOCTOR）
- [x] 验证添加成员时的错误处理（群组不存在、用户已在群组中等）
...
**任务 3.3：用户API路由实现** ✅ **已完成**

- [x] 实现 `app/api/user.py`：
  - ✅ `POST /api/user/create` - 创建用户
  - ✅ `GET /api/user/{user_id}` - 获取用户信息
  - ✅ `PUT /api/user/{user_id}/role` - 更新用户身份标签
- [x] 在 `app/main.py` 中注册用户路由：
  ```python
  from app.api import user
  app.include_router(user.router, prefix="/api")
  ```
- [x] 验证请求参数验证（使用Pydantic Schema）
- [x] 验证错误响应格式统一
...
**任务 3.4：群组API路由实现** ✅ **已完成**

- [x] 实现 `app/api/group.py`：
...
# 3. 创建群组（creator_id作为查询参数）
curl -X POST "http://localhost:8000/api/group/create?creator_id=user_xxx" \
  -H "Content-Type: application/json" \
  -d '{
    "group_name": "测试群组",
    "description": "测试描述"
...
- [x] 实现 `app/services/core/message_service.py`：
...
- [x] 验证发送消息时的错误处理（群组不存在、发送人不在群组中等）
- [x] 验证分页参数验证（page >= 1, page_size 1-100）
...
```bash
# 1. 发送消息（from_user_id作为查询参数）
curl -X POST "http://localhost:8000/api/message/send?from_user_id=user_xxx" \
  -H "Content-Type: application/json" \
  -d '{
    "group_id": "group_xxx",
    "content": "测试消息"
...
**任务 5.1：WebSocket服务层实现** ✅ **已完成**

- [x] 实现 `app/services/core/websocket_service.py`：
...
- [x] 验证使用脚手架提供的WebSocket管理器（`app/websocket/manager.py`）
- [x] 验证消息推送格式统一
...
**任务 5.2：WebSocket处理器实现** ✅ **已完成**

- [x] 实现 `app/websocket/im_handler.py`：
  - ✅ `handle_connection(websocket: WebSocket, user_id: str)` - 继承自基类
  - ✅ `handle_send_message(user_id: str, group_id: str, content: str)` - 在 `on_message` 中实现
- [x] 验证WebSocket连接管理
- [x] 验证消息接收和处理逻辑
- [x] 验证连接断开处理

**验收标准**：
- ✅ WebSocket处理器已实现（IMWebSocketHandler）
- ✅ 连接管理正常
- ✅ 消息接收和处理正常

---

**任务 5.3：WebSocket路由注册** ✅ **已完成**

- [x] 在 `app/main.py` 中注册WebSocket路由：
  ```python
  @app.websocket("/ws/{user_id}")
  async def websocket_endpoint(websocket: WebSocket, user_id: str):
      # 使用 IMWebSocketHandler 处理连接
      handler = IMWebSocketHandler(user_id)
      await handler.handle_connection(websocket)
  ```
- [x] 验证WebSocket连接可以正常建立
- [x] 验证消息可以正常推送

**验收标准**：
- ✅ WebSocket路由已注册（使用 IMWebSocketHandler）
- ✅ 连接可以正常建立
- ✅ 消息可以正常推送
...
ws.onmessage = (event) => {
  ...
  if (message.type === 'welcome') {
    ...
    console.log('消息发送成功:', message.message_id);
  }
};
