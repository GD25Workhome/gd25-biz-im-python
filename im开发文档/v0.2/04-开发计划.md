...
**任务 1.3：配置扩展（基于脚手架配置）** ⚠️ **部分完成，需要更新**

**当前状态**：
- ✅ `app/config.py` 已包含AI相关配置（`IMSettings`类）
- ⚠️ 但配置格式为旧版本（`ai_provider`, `ai_api_key`, `ai_base_url`等）
- ⚠️ 需要更新为新设计格式（`ai_service_url`, `ai_service_api_key`, `ai_service_timeout`）

**待完成任务**：
- [ ] 更新 `app/config.py`，将AI配置更新为新设计格式：
  ```python
  # AI服务配置（AI封装层使用）
...
**任务 1.4：数据库表创建（基于脚手架数据库模块）** ⚠️ **部分完成，需要更新**

**当前状态**：
- ✅ `app/models/user.py` 已存在（但字段不符合新设计，需要更新）
- ✅ `app/models/message.py` 已存在（但字段不符合新设计，需要更新）
- ✅ `app/models/ai_chat_record.py` 已存在（需要重命名为`ai_interaction_record.py`并更新字段）
- ❌ `app/models/group.py` 不存在，需要创建
- ❌ `app/models/group_member.py` 不存在，需要创建
- ❌ 数据库迁移脚本未创建

**待完成任务**：
- [ ] 更新 `app/models/user.py`，按照新设计实现：
...
- [ ] 重命名并更新 `app/models/ai_chat_record.py` 为 `app/models/ai_interaction_record.py`：
